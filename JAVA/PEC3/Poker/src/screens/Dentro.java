...
public class Dentro extends javax.swing.JFrame {
    ...
    */
    ...
    private void mesaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_mesaActionPerformed
        JugadorRegistrado jugador[]=new JugadorRegistrado[4];
        ...
        if(claves.length > 3)
        {
            ...
            {
                if(jugador[0].getNIF().equals(claves[x]))
                    prohibido=x;
            }
            for(int x=1;x<4;x++)
            {
                if((prohibido+x)<claves.length)
                    jugador[x]=CasaDeApuestas.getregistrado(claves[prohibido+x]);
                else
                    jugador[x]=CasaDeApuestas.getregistrado(claves[prohibido+x-claves.length]);
            }
            jugable=true;
        }
        ...
        {
            Mesa m = new Mesa();
            m.setJugador(jugador[0],jugador[1],jugador[2],jugador[3]);
            m.setVisible(true);
            this.setVisible(false);
            this.dispose();
        }
