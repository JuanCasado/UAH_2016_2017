    ...
    private void ButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ButtonActionPerformed
        ...
        {   
            try
            {
                ...
                {
                    throw new Ex(Ex.DINERO_INSUFICIENTE);
                }
            }
            catch (Ex e)
            {
                ...
                if (Fecha.esActualizado())
                {
                   ...
                   while (jugador.getFecha().isBefore(nfecha))
                   {
                    ...
                    try (PrintWriter salida = new PrintWriter(new BufferedWriter(new FileWriter("./src/RegistroJugadores/"+jugador.getNIF()+".txt",true)))) 
                    {
                        ...
                        CasaDeApuestas.actualizarBeneficios(jugador.getSaldo()*0.02);
        ...
        else
        {
            try
            {
                if (Double.parseDouble(dinero.getValue().toString()) <1)
                {
                    throw new Ex(Ex.DINERO_NEGATIVO);
                }
            }
            catch (Ex e)
            {
